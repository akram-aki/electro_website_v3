        ...
        <div className="bg-white p-6 rounded shadow">
            <div className="flex justify-between items-center mb-6">
                <div className="flex items-center gap-4">
                    ...
                    <h2 className="text-2xl font-bold text-[#70a939]">{form.title} - Submissions</h2>
                </div>
                <button 
                    ...
                    <table className="min-w-full bg-white border border-gray-200">
                        <thead className="bg-gray-50">
                            <tr>
                                ...
                                {fields.map(field => (
                                    <th key={field.id} className="py-3 px-4 border-b text-left text-xs font-bold text-gray-500 uppercase tracking-wider">
                                        {field.label}
                                    </th>
                                ))}
                            </tr>
                        ...
                        <tbody className="divide-y divide-gray-200">
                            {submissions.map(submission => (
                                <tr key={submission.id} className="hover:bg-gray-50">
                                    ...
                                    {fields.map(field => (
                                        <td key={field.id} className="py-4 px-4 text-sm text-gray-900">
                                            {/* Handle different value types if necessary */}
                                            {typeof submission.data[field.label] === 'boolean' 
                                                ? (submission.data[field.label] ? 'Yes' : 'No')
